...
API_ENDPOINT = "https://iot-backend-url.azurewebsites.net/api/registro"
...
while True:    
    ...
    cv2.line(Frame, (0,int(CoorYEntranceLine)), (width,int(CoorYEntranceLine)), (255, 0, 0), 2)
    cv2.line(Frame, (0,int(CoorYExitLine)), (width,int(CoorYExitLine)), (0, 0, 255), 2)
    ...
    for c in cnts:
        ...
        if (CheckEntranceLineCrossing(CoordYCentroid,CoorYEntranceLine,CoorYExitLine)):
            EntranceCounter += 1
            data = {'raspberry':1, 'datetime':current_time, 'cantidad': 1} 
